...
// Avatar component for profile nav - moved outside to prevent re-creation
interface ProfileAvatarProps {
    user: User | null;
    isLoggedIn: boolean;
}

const ProfileAvatar: React.FC<ProfileAvatarProps> = ({ user, isLoggedIn }) => {
    ...
    return (
        <nav className="h-20 bg-black/80 backdrop-blur-md border-t border-white/10 z-40 flex justify-around items-center">
            ...
            <NavLink to={`/${locale}/profile/me`} className={navLinkClass}>
                <ProfileAvatar user={user} isLoggedIn={isLoggedIn} />
